...
**Answer: It says: "Warning: One or more parsing issues, call \`problems()\` on your data frame for details" which means if you run the function 'problems()' it will explain what is going wrong. It explains that it expected something different than what it saw, which caused the error message.**
...
**Answer: There are no more parsing errors, but the delimiter message is still displaying.**
...
**Answer: "Amount" is a dbl data type, while "Date" is a chr data type.**
...
**Answer: There are 549 different zip codes which are under 5 digits. These could be mistypes (for example, zip code 2241 was supposed to be 22411).**
...
**Answer: Some of the Categories are entirely blank, while others aren't written out all the way (such as 'L' instead of 'Loan').**
...
**Answer: There is a pretty severe difference between the beginning of the list and even just the third page. While the third page is in the thousands, the first two loans are below \$100.**
...
**Answer: It made them lowercase with no spaces in between (aka Fiscal Period became fiscal_period)**
...
**Answer: There are 50 possible duplicates that the code returned.**
...
**Answer: The new dataframe has 17,740 rows, which is lower than the previous number of rows.**
...
**Answer: Some zip codes were previously extended versions of the actual zip code they were asking for, like 21668-1224. This code took only the first 5 digits, changing it to 21668.**
...
**Answer: The Maryland Tourism Board loaned out 700,000 to the zip code 21202, which does make sense to me.**
