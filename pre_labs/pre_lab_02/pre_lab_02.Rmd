...
**Answer: It could be important for future cases of working with these results. If you don't make a variable you can't really work on manipulating it or working with it in general, so the variable is useful for the future.**
...
**Answer: "Or" adds all classes in the theatre department with *all* classes that have 15 seats or more, while and restricts it to classes that fit both of these criteria.**
...
**Answer: It changed the entire date column from m/d/y to y-m-d, including adding zeros which previously were not there for clarity in each month and day.**
...
**Answer: I think the month column is essentially saying the first day of that month to the last day of that month is being counted. For example, the first data point goes from June 01, 2022 through June 30, 2022 and anything that falls in that interval is included in total_amount.**
...
**Answer: I believe here we made a total_votes variable which added up all the different votes for that election cycle and then created a new variable, pct_moore, which is calculating the percentage of votes Wes Moore actually got by dividing Moore's total votes by our new variable.**
...
**Answer: For some reason I am unable to see tasks 10-12 on R, so this answer is just based on what I think the code would do. For this, it rearranged the percentages in descending order, meaning that the highest percentage is first. This gives order to the data and helps sort through it.**
...
**Answer: This ordered it in ascending order, meaning the column's lowest percentage is first. I do think it is fairly interested to see which areas didn't vote for the state's current governor, and it shows who his real competition in the race was.**
...
**Answer: There is also str_to_title() which only capitalizes the first letter of each word, and str_to_lower() which makes all characters lowercase. For addresses, pm_state_any() finds if an address contains any state name or abbreviation whatsoever.**
...
**Answer: This code block is working to find the state for each submission and creating a column with the abbreviation. To do this, the code clarifies that Maryland = MD, California = CA, and so on.**
...
**Answer: I think so. I believe this is essentially using the state variable we made previously to find how much of the expenses came from each state, and then going in descending order. This means the expenses primarily came from Maryland, and were least likely to come from West Virginia.**
...
1.  I could ask something along the lines of how did the expenditures from 2021 to 2022 change.

2.  How many expenses came from the same city/zipcode?

3.  What percentage of the expenses came from each payee? Who paid the most? The least?
