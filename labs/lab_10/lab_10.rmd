...
#Create the map
ggplot() +
  geom_sf(data = md_statewide_geometry, aes(fill = moore_brown_difference)) + theme_minimal() + scale_fill_gradient2()
...
county_outages <- county_outages |>
  mutate(outage_rates = outage_customers/total_customers) |>
  mutate(rate_per_10000 = outage_rates*10000)
```

```{r}
#Merge geometries with outage data
outage_geometry <- md_counties |>
...
#Map it out
ggplot() +
  geom_sf(data = outage_geometry, aes(fill = rate_per_10000)) + theme_minimal() + scale_fill_gradient2()
```
...
ev_geometry <- md_counties |>
  inner_join(ev_data, by=c('NAME' = 'county'))
```

```{r}
#Map out the change
ggplot() +
  geom_sf(data = ev_geometry, aes(fill = pct_change)) + theme_minimal() + scale_fill_gradient2()
```
...
ggplot() +
  geom_sf(data = ev_geometry_with_pop, aes(fill = per_capita)) + theme_minimal() + scale_fill_gradient2()
```
