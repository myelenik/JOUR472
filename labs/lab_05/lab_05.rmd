...
```{r}
baltimore_county <- read_csv("tabula-baltimore_county.csv", col_names = FALSE) |>
  clean_names() |>
...
**A1. This data does not appear to support the idea that there are days with no overdose calls, given that there are 366 rows for one year. However, I am confused about why it says 366 instead of 365.**
...
**A2. This shows that the majority of calls happen on the weekend/Monday, and they decrease during weekdays (Tues-Fri).**
...
**A3. I did notice that of the first three, two of them are police departments. This skews the data a bit as they were taken to the department rather than coming from it. The syntax of the data does give me pause as some addresses are not full addresses, while other could be written differently.**
...
**A4. I would want to figure out what is going on with 4540 Silver Spring Road and why it has almost triple the amount of overdoses the second most frequent address does.**
